...
def index():
    ...
    # Top 5 category for news message
    top_category_count_news= df[df['genre'] == 'news'].iloc[:,4:].sum().sort_values(ascending=False)[1:6]
    top_category_names_news = list(top_category_count_news.index)
    ...
    graphs = [
        ...
        },
        {
            'data': [
                Bar(
                    x=top_category_names_news,
                    y=top_category_count_news
                )
            ],
